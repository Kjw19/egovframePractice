...
public class MyFirstAspect {
	 ...
	 * @Before
	 * 
	 * After Returing Advice	대상 객체의 메서드가 예외 없이 실행한 이후에
	 * @AfterReturning			공통 기능을 실행
	 * 
	 * After Throwing Advice	대상 객체의 메서드를 실행하는 도중 예외가 발생한
	 * @AfterThrowing			경우에 공통 기능 실행
	 * 
	 * After Advice				대상 객체의 메서드를 실행하는 도중 예외가 발생했
	 * @After					는지 여부와 상관없이 메서드 실행 후 공통 기능 실행
	 * 							(try~catch~finally의 finally블럭과 비슷)
	 * 
	 * Around Adivce			대상 객체의 메서드 실행 전, 후 또는 예외 발생 시점에
	 * @Around					공통 기능을 실행
	 * 
	 * */
	...
	public String around(
		...
		try {
			...
			System.out.println(
					"Hello Around after! **메서드가 호출된 후에 나온다."
					+ "반환된 객체 : " + s);
		}
		return s;
	};
